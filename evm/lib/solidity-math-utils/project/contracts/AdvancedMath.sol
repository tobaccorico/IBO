...
library AdvancedMath {
    ...
    uint256 internal constant LAMBERT_NEG2_T_MASK = 0xffffffffffffffffffffffffffffffffff;
    uint256 internal constant LAMBERT_POS1_MAXVAL = 0x002f16ac6c59de6f8d5d6f63c1482a7c86;
    uint256 internal constant LAMBERT_POS2_MAXVAL = 0x0c2f16ac6c59de6f8d5d6f63c1482a7c56;
    ...
    */
    function lambertNeg2(uint256 x) internal pure returns (uint256) { unchecked {
        bytes memory values = LAMBERT_NEG2_VALUES;
        uint256 y = x - LAMBERT_NEG1_MAXVAL - 1;
        uint256 i = y / LAMBERT_NEG2_SAMPLE;
        uint256 a = LAMBERT_NEG2_SAMPLE * (i + 0);
        uint256 b = LAMBERT_NEG2_SAMPLE * (i + 1);
        uint256 c = LAMBERT_NEG2_T_SIZE * (i + 1);
        uint256 d = LAMBERT_NEG2_T_SIZE * (i + 2);
        uint256 e = read(values, c) & LAMBERT_NEG2_T_MASK;
        uint256 f = read(values, d) & LAMBERT_NEG2_T_MASK;
        uint256 g = e * (b - y);
        uint256 h = f * (y - a);
