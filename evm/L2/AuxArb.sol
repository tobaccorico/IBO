...
contract AuxArb is // Auxiliary
    ...
    function getFee(address token)
        ...
        return BasketLib.calcFeeWithPairs(idx, deps,
            BasketLib.StakedPairs({ base: [4, 5, 6, 7],
                                  staked: [8, 10, 9, 11] }),
                                             stables, jury);
    }
    ...
    function _take(address who, uint amount, address token,
        ...
        if (amount > 0) { // when i'm a loan with you
            ...
            if (amounts[1] == 0 || amount == 0) return sent; // redempt
    ...
    function _executeProRata(address who, uint amount,
        ...
        bool strict) private returns (uint sent) { // bitmasked withdrawal
        uint[12] memory w = BasketLib.calcWithdrawAmounts(amount, amounts,
            indexToSkip, strict, [_getPrice(9), _getPrice(10), _getPrice(11)],
